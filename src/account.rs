...
impl Account {
    ...
    async fn get_files_helper(&self) -> Vec<Rc<RefCell<CourseFile>>> {
        ...
        let result_a: Vec<Rc<RefCell<CourseFile>>> = join_all(self.folders.iter().map(|folder| async move {
        ...
        for file in result_a {
            if let Some(file_filter) = file_filter_option {
                ...
                if file_filter_check(
                    file_filter,
                    file.borrow().size,
                    file_extension,
                    &file.borrow().content_type,
                    &file_name_str,
                    &mut denied_files,
                ) {
        ...
        let result_b: Vec<Rc<RefCell<CourseFile>>> = join_all(self.assignmnets.iter().map(|assignment| async move {
            let path = if !self.config.allow_term {
                self.config.local_place.clone()
            } else {
                self.config
        ...
        let result_c: Vec<Rc<RefCell<CourseFile>>> = join_all(self.pages.iter().map(|page| async move {
            let path = if !self.config.allow_term {
                self.config.local_place.clone()
            } else {
